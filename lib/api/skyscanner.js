...
var NO_FLIGHT_EN_US = 'There is no direct flight found for the specified dates. Please try again with other dates.';
var NO_FLIGHT_NL_NL = 'Er zijn helaas geen directe vluchten gevonden voor de opgegeven data. Probeer het aub nogmaals met andere data.';

function pollSkyscanner(poll_url) {
...
module.exports = {
  ...
  getFlightDetails: function(params, callback) {
    ...
    var outboundDate = new Date(params.outbounddate + year);
    var outBoundMonth = new String(outboundDate.getMonth() + 1).length < 2 ? '0'.concat((outboundDate.getMonth() + 1)) : (outboundDate.getMonth() + 1);
    var outBoundDay = new String(outboundDate.getDate()).length < 2 ? '0'.concat(outboundDate.getDate()) : outboundDate.getDate();

    debug('outbounddate: ' + year + '-' + outBoundMonth + '-' + outBoundDay);

    var inboundDate = new Date(params.inbounddate + year);
    var inBoundMonth = new String(inboundDate.getMonth() + 1).length < 2 ? '0'.concat((inboundDate.getMonth() + 1)) : (inboundDate.getMonth() + 1);
    var inBoundDay = new String(inboundDate.getDate()).length < 2 ? '0'.concat(inboundDate.getDate()) : inboundDate.getDate();

    debug('outbounddate: ' + year + '-' + inBoundMonth + '-' + inBoundDay);
    ...
    request({
      ...
      if (poll_url === undefined) {
        callback(null, {text: [NO_FLIGHT_NL_NL]});
      } else {
        var inProgress = true;
            ...
            client.get(poll_url, args, function(data, response) {
              ...
              if (data.Status == 'UpdatesComplete') {
                ...
                if (data.Itineraries[0] === undefined) {
                  ...
                  callback(null, {text: ['We hebben een vlucht gevonden voor <b>€' + Math.round(price) + '</b>.',
                                        ' ',
                                        'Uitgaand: ' + outboundCarrierCode + outboundFlightNumber + ' op ' + outboundDepature.substring(0,10) + ', ' + params.origin + ' ' + outboundDepature.substring(11,16) + ' -> ' + params.destination + ' ' + outboundArrival.substring(11,16),
                                        'Operator: <i>' + outboundOperatorCarrierName + '</i>',
                                        ' ',
                                        'Inkomend: ' + inboundCarrierCode + inboundFlightNumber + ' op ' + inboundDepature.substring(0,10) + ', ' + params.destination + ' ' + inboundDepature.substring(11,16) + ' -> '+ params.origin + ' ' + inboundArrival.substring(11,16),
                                        'Operator: <i>' + inboundOperatorCarrierName + '</i>',
                                        ' ',
                                        'Deze service is powered by <b>IBM Watson</b> en Skyscanner.',
                                        'Ga naar <a href=\"http://skyscanner.net\">skyscanner.net</a> voor het boeken van deze vlucht.'
                  ]});
                }
              }
            })
